...
namespace CourseService.Presentation.Controllers
{
    ...
    public class AcademicProgramController(IService<AcademicProgram, int> academicProgramService)
        : ControllerBase
    {
        protected readonly IService<AcademicProgram, int> _academicProgramService =
        ...
        public async Task<IActionResult> Create([FromBody] AcademicProgramDTO academicProgram)
        {
            if (academicProgram is null)
                return BadRequest();
            var newAcademicProgram = new AcademicProgram { Name = academicProgram.Name };
            var createdAcademicProgram = await _academicProgramService.Create(newAcademicProgram);
            return CreatedAtAction(
                nameof(GetById),
                new { id = createdAcademicProgram.Id },
        ...
        public async Task<IActionResult> Update(
            int id,
            [FromBody] AcademicProgramDTO academicProgram
        )
        {
            if (academicProgram is null)
                return BadRequest();
            var newAcademicProgram = new AcademicProgram { Id = id, Name = academicProgram.Name };
            var updatedAcademicProgram = await _academicProgramService.Update(
                id,
                newAcademicProgram
            );
